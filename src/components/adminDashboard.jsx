...
export default function AdminDashboard({ setView }) {
  ...
  const [togglingKey, setTogglingKey] = useState(null); // tracks which button is loading
  ...
  const updateBooking = async (id, updates) => {
    ...
    try {
      ...
      fetchData(); // Rollback by re-fetching real data
    }
  };

  const toggleStatus = async (key) => {
    if (togglingKey) return; // Prevent double-clicks
    ...
    <div className="admin-container" style={{ padding: '20px', background: '#fff', minHeight: '100vh' }}>
      <nav style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #eee', paddingBottom: '10px', marginBottom: '20px' }}>
        <h2 style={{ fontFamily: 'Playfair Display, serif' }}>
          ROYAL 'N' PANEL {isUpdating && <small style={{ fontSize: '0.8rem', color: 'orange' }}>...Saving</small>}
        </h2>
